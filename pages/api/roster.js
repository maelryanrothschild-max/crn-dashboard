...
import { requireUser, safeUser, isOwnerModerator, getRoster, canManageEmployee } from "../../lib/auth";
...
function visibleRoster(user,roster){if(isOwnerModerator(user))return roster.map(safeUser);if(user.role==="director"){if(isRegionalDirector(user))return roster.filter(e=>norm(e.brand)===norm(user.brand)).map(safeUser);if(!user.store||user.store==="Все магазины")return[safeUser(user)];return roster.filter(e=>e.store===user.store).map(safeUser);}return roster.filter(e=>String(e.id)===String(user.id)).map(safeUser);}
...
function cleanEmployeeInput(emp,user){
  ...
  if(!isOwnerModerator(user)&&!DIRECTOR_ADD_ROLES.has(role))return{error:"Директор может добавлять стилистов, администраторов и онлайн-менеджеров"};
...
export default async function handler(req,res){
 try{
  ...
  if(!(isOwnerModerator(user)||user.role==="director"))return res.status(403).json({error:"Недостаточно прав"});
  ...
  if(action==="remove"){
    ...
    if(!canManageEmployee(user,target))return res.status(403).json({error:"Можно удалять только сотрудников своей команды. Учётные записи директоров и модератора защищены."});
  ...
  if(!isOwnerModerator(user))return res.status(403).json({error:"Только модератор может изменять данные существующих сотрудников"});
  if(action==="update"){
