...
function getSecret() {
  const secret = process.env.SESSION_SECRET || process.env.UPSTASH_REDIS_REST_TOKEN;
...
export async function getRoster() {
  const roster = await redis.get("roster");
  return roster || DEFAULT_ROSTER;
}
...
export async function getCurrentUser(req) {
  ...
  return roster.find((e) => String(e.id) === String(data.uid)) || null;
}
