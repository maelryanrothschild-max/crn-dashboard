...
function applyOperationalRosterOverrides(roster) {
  ...
  const next = (roster || []).map((e) => {
    if (String(e.id) === "2016" || (String(e.surname || "").trim().toLowerCase() === "зикранец" && String(e.firstname || "").trim().toLowerCase() === "эльвира")) {
      const patch = {
        ...e,
        id: String(e.id || "2016"),
        role: "director",
        city: "Алматы",
        store: "Corneli Almaty Outlet",
        brand: "Corneli",
      };
      if (e.role !== patch.role || e.city !== patch.city || e.store !== patch.store || e.brand !== patch.brand) changed = true;
      return patch;
    }
    return e;
  });
